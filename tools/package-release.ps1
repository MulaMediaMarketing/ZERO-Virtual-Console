...
$manifest=[ordered]@{schema=5;product="ZERO Virtual Console";milestone="M1 / Runtime V4.1 / Production Architecture";commit=$commit;generated_at_utc=[DateTime]::UtcNow.ToString("o");dependency_result=$dependency.result;dependency_model=$dependency.dependency_model;architecture_result=$architecture.result;automated_acceptance_result=$acceptance.automated_result;architecture_metrics=$architecture.source_metrics;acceptance_contract_metrics=$architecture.acceptance_metrics;acceptance_performance_metrics=$acceptance.performance_metrics;shell_metric=$architecture.shell_metric;rc_qualified=$false;rc_qualification_note="Bundle is release-candidate material only until physical Windows 11 x64 and controller qualification passes for this exact commit.";files=$manifestEntries.ToArray()}
