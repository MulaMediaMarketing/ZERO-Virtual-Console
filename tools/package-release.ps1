...
$zipPath=Join-Path (Split-Path -Parent $OutputDir) "ZERO-Player-Windows-x64-RC.zip";if(Test-Path $zipPath){Remove-Item -Force $zipPath};Compress-Archive -Path (Join-Path $OutputDir '*') -DestinationPath $zipPath -CompressionLevel Optimal;Require-File $zipPath
