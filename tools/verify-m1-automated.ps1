...
function Invoke-AcceptanceStep {
  ...
  $results.Add([pscustomobject]@{
    name=$Name; category=$Category; status=$status; exit_code=$exitCode
    started_at_utc=$started.ToString("o"); ended_at_utc=$ended.ToString("o")
    duration_ms=[int][Math]::Round(($ended-$started).TotalMilliseconds); output=$output
  })
...
$payloadContractPassed = $manifest.package_id -eq "zero.system.reference" -and
  $manifest.minimum_runtime_major -eq 4 -and $manifest.zero_resume -eq $true -and
  $manifest.zero_achievements -eq $true -and $manifest.zero_overlay -eq $true -and
...
$results.Add([pscustomobject]@{
  name="reference_package_contract"; category="reference_package"; status=$referenceStatus
  exit_code=$(if($payloadContractPassed){0}else{2}); started_at_utc=$referenceTimestamp
  ended_at_utc=$referenceTimestamp; duration_ms=0
...
$results.Add([pscustomobject]@{
  name="automated_runtime_budget"; category="performance"; status=$(if($runtimeBudgetPassed){"PASS"}else{"FAIL"})
  exit_code=$(if($runtimeBudgetPassed){0}else{2}); started_at_utc=$budgetTimestamp; ended_at_utc=$budgetTimestamp
  duration_ms=0; output="Measured acceptance duration: $totalDurationMs ms; budget: $automatedRuntimeBudgetMs ms."
})
...
$report = [ordered]@{
  schema=4; generated_at_utc=[DateTime]::UtcNow.ToString("o")
  zero_milestone="M1 / Runtime V4.1 / Production Architecture"; commit=$commit
  automated_result=$automatedResult; rc_qualified=$false
  ...
  performance_metrics=[ordered]@{
    total_acceptance_duration_ms=$totalDurationMs; runtime_budget_ms=$automatedRuntimeBudgetMs
    runtime_budget_passed=$runtimeBudgetPassed; slowest_gate=$(if($slowest){$slowest.name}else{"none"})
    slowest_gate_duration_ms=$(if($slowest){$slowest.duration_ms}else{0})
  }
  binaries=[ordered]@{shell=$zeroShell;acceptance=$zeroAcceptance;reference_game=$referenceExe}
  automated_checks=$results.ToArray(); physical_qualification_required=$hardwareRequirements
}
...
$lines.Add("| --- | --- | --- | ---: |")
foreach ($item in $results) { $lines.Add("| $($item.name) | $($item.category) | $($item.status) | $($item.duration_ms) |") }
$lines.Add("")
$lines.Add("## Physical qualification still required")
foreach ($item in $hardwareRequirements) { $lines.Add("- **$($item.name)** — $($item.reason)") }
$lines | Set-Content -Path $markdownPath -Encoding UTF8

Write-Host "ZERO M1 automated acceptance report: $jsonPath"
Write-Host "ZERO M1 automated acceptance: $automatedResult"
