...
bool packagePath(const std::filesystem::path& root,const std::string& relative,std::filesystem::path& out,std::wstring& error){if(relative.empty()){out={};return true;}const auto converted=PathFromUtf8(relative);if(!converted){error=L"ZERO manifest contains an invalid UTF-8 filesystem path.";return false;}out=root / *converted;return true;}
}
