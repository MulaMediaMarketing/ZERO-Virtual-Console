...
class Parser {
public:
    explicit Parser(std::string_view text, size_t maxDepth = 64) noexcept : text_(text), maxDepth_(maxDepth) {}
...
private:
    ...
    bool ParseRawUtf8(unsigned char first, std::string& out) {
        if (first < 0x80) { out.push_back(static_cast<char>(first)); return true; }
        int continuation = 0;
        uint32_t cp = 0;
        uint32_t min = 0;
        if ((first & 0xE0) == 0xC0) { continuation = 1; cp = first & 0x1F; min = 0x80; }
        else if ((first & 0xF0) == 0xE0) { continuation = 2; cp = first & 0x0F; min = 0x800; }
        else if ((first & 0xF8) == 0xF0) { continuation = 3; cp = first & 0x07; min = 0x10000; }
        else return Fail("invalid UTF-8 leading byte");
        std::string bytes;
        bytes.push_back(static_cast<char>(first));
        for (int i = 0; i < continuation; ++i) {
            if (pos_ >= text_.size()) return Fail("truncated UTF-8 sequence");
            const unsigned char next = static_cast<unsigned char>(text_[pos_++]);
            if ((next & 0xC0) != 0x80) return Fail("invalid UTF-8 continuation byte");
            bytes.push_back(static_cast<char>(next));
            cp = (cp << 6) | (next & 0x3F);
        }
        if (cp < min || cp > 0x10FFFF || (cp >= 0xD800 && cp <= 0xDFFF)) return Fail("invalid UTF-8 code point");
        out += bytes;
        return true;
    }
    ...
    bool ParseString(std::string& out) {
        ...
        while (pos_ < text_.size()) {
            ...
            if (c >= 0x80) {
                if (!ParseRawUtf8(c, out)) return false;
                continue;
            }
            ...
            switch (esc) {
                ...
                case 'u': {
                    ...
                    } else if (first >= 0xDC00 && first <= 0xDFFF) return Fail("unexpected low surrogate");
                    AppendUtf8(cp, out);
                    break;
                }
    ...
    bool ParseNumber(Value& out) {
        ...
        if (Consume('0')) {
            if (pos_ < text_.size() && text_[pos_] >= '0' && text_[pos_] <= '9') return Fail("leading zero in number");
        } else {
        ...
        if (Consume('.')) {
            ...
            while (pos_ < text_.size() && text_[pos_] >= '0' && text_[pos_] <= '9') ++pos_;
            if (pos_ == digits) return Fail("fraction requires digits");
        }
        if (pos_ < text_.size() && (text_[pos_] == 'e' || text_[pos_] == 'E')) {
            ...
            const size_t digits = pos_;
            while (pos_ < text_.size() && text_[pos_] >= '0' && text_[pos_] <= '9') ++pos_;
            if (pos_ == digits) return Fail("exponent requires digits");
        }
        const auto token = text_.substr(start, pos_ - start);
        if (!floating) {
            int64_t value = 0;
            const auto parsed = std::from_chars(token.data(), token.data() + token.size(), value);
            if (parsed.ec == std::errc{} && parsed.ptr == token.data() + token.size()) { out.storage = value; return true; }
        }
    ...
    bool ParseValue(Value& out, size_t depth) {
        ...
        switch (text_[pos_]) {
            ...
            case '"': { std::string s; if (!ParseString(s)) return false; out.storage = std::move(s); return true; }
            ...
            default: if (text_[pos_] == '-' || (text_[pos_] >= '0' && text_[pos_] <= '9')) return ParseNumber(out); break;
        }
        return Fail("invalid JSON value");
    }
};

inline ParseResult Parse(std::string_view text, size_t maxDepth = 64) { return Parser(text, maxDepth).Parse(); }
...
inline const std::string* String(const Object& object, std::string_view key) noexcept {
    const auto* value = Find(object, key); return value ? value->AsString() : nullptr;
}
inline std::optional<int64_t> Integer(const Object& object, std::string_view key) noexcept {
    const auto* value = Find(object, key); if (value) if (const auto* v = value->AsInt()) return *v; return std::nullopt;
}
inline std::optional<bool> Boolean(const Object& object, std::string_view key) noexcept {
    const auto* value = Find(object, key); if (value) if (const auto* v = value->AsBool()) return *v; return std::nullopt;
}
